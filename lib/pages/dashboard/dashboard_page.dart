...
class DashboardPage extends ConsumerWidget {
  ...
  void _showDropdownCard(
    ...
    final notifier = ref.read(serverStatusProvider.notifier);
    final hasCurrent =
        currUrl.isNotEmpty && servers.any((s) => s.url == currUrl);
                          ...
                          GestureDetector(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                const Text(
                                  '添加',
                                  style: TextStyle(color: Color(0xFF0062F5)),
                                ),
                              ],
                            ),
                          ),
                    ...
                    child: ElevatedButton(
                      onPressed: () async {
                        ...
                        if (kIsWeb) {
                          ...
                          Navigator.pop(ctx);
                          ScaffoldMessenger.of(ctx).showSnackBar(
                            SnackBar(content: const Text('已保存, 点击切换即可连接')),
                          );
                          return;
                        }
                        final err = await ref
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _sysInfoRow(
                          '发行版本',
                          data.platform.isNotEmpty ? data.platform : '-',
                        ),
                        const SizedBox(height: 8),
                        _sysInfoRow(
                          '内核版本',
                          data.kernelVersion.isNotEmpty
                              ? data.kernelVersion
                              : '-',
                        ),
                        const SizedBox(height: 8),
                        ...
                        _sysInfoRow(
                          '主机地址',
                          data.ipv4Address.isNotEmpty ? data.ipv4Address : '-',
                        ),
                        const SizedBox(height: 8),
