          ...
          child: list.isEmpty
              ...
              : RefreshIndicator(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (ctx, i) => _ImageTile(image: list[i]),
                  ),
                ),
        ),
      ],
          ...
          FilledButton(
            ...
            onPressed: () {
              ...
              ScaffoldMessenger.of(
                context,
              ).showSnackBar(const SnackBar(content: Text('正在清理...')));
              ref.read(imageListProvider.notifier).prune(all: true).then((
                result,
              ) {
                if (context.mounted) {
                  ScaffoldMessenger.of(
                    context,
                  ).showSnackBar(SnackBar(content: Text('清理完成: $result')));
                }
                ref.read(imageListProvider.notifier).refresh();
              });
            },
...
class _PullProgressDialog extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_PullProgressDialog> createState() =>
      _PullProgressDialogState();
}
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(
            ...
            onPressed: () {
              Navigator.pop(ctx);
              ref.read(imageListProvider.notifier).remove([image.id]);
            },
            child: const Text('删除'),
          ),
        ],
